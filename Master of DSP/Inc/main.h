...
/* Constants -----------------------------------------------------------------*/
#define NUM_AM_CHANNELS 120
#define ADC_BUF_LENG 4096			// Must be an int and a power of 2 (min 8)
#define BITS_UPSCALE_FFT 10 		// If NUM_SAMPLES is 2048, should be 10, if NUM_SAMPLES is 4096, should be 11
#define NUM_DAC_BUFS 2 				// Number of DEMODULATE_OUT_NUM_SAMPLES
#define FFT_FILTERED_ZOOM 2			// (Either x1 or x2. If more required, the arm_rfft function cannot be used.) The zoom of the filtered FFT vs the unfiltered FFT.
#define DOWNSAMPLING_FACTOR 64 		// Number of input samples per output sample.
#define INPUT_VOLTAGE_RANGE 3.3  	// The max peak-to-peak voltage that the ADC can read
#define SAMPLE_FREQ 3750000			// Samples per second (SPS)
#define ADC_NUM_BITS 12				// Number of bits that each ADC sample consists of


#define NUM_SAMPLES ADC_BUF_LENG/2	// Half the buffer is used at a time
#define NUM_OUT_SAMPLES DAC_BUF_LENG/2
#define DEMODULATE_OUT_NUM_SAMPLES NUM_SAMPLES/DOWNSAMPLING_FACTOR
#define DAC_BUF_LENG DEMODULATE_OUT_NUM_SAMPLES*NUM_DAC_BUFS
#define INPUT_VOLTAGE_OFFSET = INPUT_VOLTAGE_RANGE/2.0  // DC voltage offset
